...
SIAMotionPlanner::SIAMotionPlanner(ros::NodeHandle &nh)
    : nh_(nh)
{
    ...
    nh_.getParam("kinova_driver_tool_pose_topic", sia_driver_tool_pose_topic_);
    ...
    ee_traj_srv_ = nh_.advertiseService("/ee_traj_srv", &SIAMotionPlanner::EeTrajCallback, this);
    joint_traj_srv_ = nh_.advertiseService("/joint_traj_srv", &SIAMotionPlanner::JointTrajCallback, this);
    ee_pose_srv_ = nh_.advertiseService("/ee_pose_srv", &SIAMotionPlanner::EePoseCallback, this);
    ee_rpy_srv_ = nh_.advertiseService("/ee_rpy_srv", &SIAMotionPlanner::EeRpyCallback, this);
    ee_delta_srv_ = nh_.advertiseService("/ee_delta_srv", &SIAMotionPlanner::EeDeltaCallback, this);
}

void SIAMotionPlanner::init()
{
	// MoveIt
...
SIAMotionPlanner::~SIAMotionPlanner() { delete group_; }

bool SIAMotionPlanner::EePoseCallback(sia_7f_arm_train::EePose::Request& req,
                                      sia_7f_arm_train::EePose::Response& res)
{
    res.pose = group_->getCurrentPose().pose;
    return true;
}
bool SIAMotionPlanner::EeRpyCallback(sia_7f_arm_train::EeRpy::Request& req, 
...
bool SIAMotionPlanner::EeTrajCallback(sia_7f_arm_train::EeTraj::Request& req,
...
bool SIAMotionPlanner::EeDeltaCallback(sia_7f_arm_train::EeDelta::Request& req,
...
bool SIAMotionPlanner::JointTrajCallback(sia_7f_arm_train::JointTraj::Request& req,
...
std::vector<double> SIAMotionPlanner::getCurrentJointState()
{
    if (debug_print_) {
        std::cout << "Enter SIAMotionPlanner::getCurrentJointValues" << std::endl;
    }
    ...
    if (debug_print_) {
        ...
        std::cout << "Leave SIAMotionPlanner::getCurrentJointValues" << std::endl;
    }
...
geometry_msgs::Pose SIAMotionPlanner::getCurrentPoseFromMoveit() { return group_->getCurrentPose().pose; }

geometry_msgs::Pose SIAMotionPlanner::getCurrentPoseFromDriver()
{
	if (debug_print_)
	{
		std::cout << "Enter SIAMotionPlanner::getCurrentPoseFromDriver" << std::endl;
	}

	geometry_msgs::PoseStampedConstPtr tool_pose =
	    ros::topic::waitForMessage<geometry_msgs::PoseStamped>(sia_driver_tool_pose_topic_, nh_, ros::Duration(1.0));
	if (!tool_pose)
	...
	if (debug_print_)
	{
		...
		std::cout << "Leave SIAMotionPlanner::getCurrentPoseFromDriver" << std::endl;
	}

	return tool_pose->pose;
}

std::vector<double> SIAMotionPlanner::getCurrentJointStateFromMoveit() { return group_->getCurrentJointValues(); }

std::vector<double> SIAMotionPlanner::getCurrentRPY() { return group_->getCurrentRPY(); }

std::size_t SIAMotionPlanner::getPlanPointNum(const moveit::planning_interface::MoveGroupInterface::Plan& plan)
{
    moveit_msgs::RobotTrajectory trajectory = plan.trajectory_;
    return trajectory.joint_trajectory.points.size();
}

bool SIAMotionPlanner::executePlan(const moveit::planning_interface::MoveGroupInterface::Plan& plan)
{
    ros::Time start_time = ros::Time::now();
    group_->execute(plan);
...
void SIAMotionPlanner::confirmToAct()
{
...
void SIAMotionPlanner::confirmToAct(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal,
...
void SIAMotionPlanner::confirmToAct(const geometry_msgs::Pose &goal, const std::string &str)
{
	std::cout << "\n"
	          << "=================MOVE TO " + str + "=================="
	          << "\n";
...
void SIAMotionPlanner::confirmToAct(const geometry_msgs::Pose &goal)
{
	ROS_INFO_STREAM("Move to target" << goal);
	std::cout << "Confirm start ---> goal info, press n to start plan" << std::endl;
	if (confirm_act_)
...
void SIAMotionPlanner::setJointValueTarget(const std::vector<double> joint_values)
{
...
void SIAMotionPlanner::setJointValueTarget(const int joint_index, const double joint_values)
{
...
void SIAMotionPlanner::setAbsoluteJointValueTarget(const int joint_index, const double joint_values)
{
	std::vector<double> current_joint_values = getCurrentJointStateFromMoveit();
	current_joint_values[joint_index] = joint_values;
	setJointValueTarget(current_joint_values);
...
void SIAMotionPlanner::moveToTargetNamed(const std::string &target_name)
{
	group_->setStartStateToCurrentState();
...
void SIAMotionPlanner::moveToTargetBestTime(const geometry_msgs::Pose &target)
{
	group_->setStartStateToCurrentState();
...
void SIAMotionPlanner::moveToTargetBestTime(const geometry_msgs::PoseStamped &target)
{
	moveToTargetBestTime(target.pose);
}

void SIAMotionPlanner::moveLineTarget(double distance_x, double distance_y, double distance_z)
{
...
void SIAMotionPlanner::moveLineTarget(const geometry_msgs::Pose &goal)
{
...
void SIAMotionPlanner::moveLineTarget(const geometry_msgs::Pose &start, const geometry_msgs::Pose &goal)
{
	std::cout << "Begin cartesian line plan by MoveIt computeCartesianPath ..." << std::endl;
...
double SIAMotionPlanner::cartesionPathPlanner(double distance_x, double distance_y, double distance_z, double roll,
                                              double pitch, double yaw, int number_point, int number_distance)
{
	...
	* @param yaw Angle around Z*/
    ...
    jointName << sia_arm_joint_names_[k];
	follow_joint_traj_goal.trajectory.joint_names.push_back(jointName.str());
	moveit_robot_traj_msg.joint_trajectory.joint_names.push_back(jointName.str());
	}
...
double SIAMotionPlanner::cartesionPathPlanner(double distance_x, double distance_y, double distance_z, int number_point, int number_distance)
{
	std::cout << "cartesion path planner..." << std::endl;
...
double SIAMotionPlanner::cartesionPathPlanner(double distance_x, double distance_y, double distance_z)
{
